...
# covsGene = SlicedData$new(t(cbind(mod[,-1], pcsGene, pd$DxNum)))
...
# save(meGeneCis, nsvGene, pcsGene, file="rdas/gene_eqtl_szControl_cisOnly.rda")
...
pdFetal= pd[fIndex,]
pcaFetal = prcomp(t(log2(geneRpkm[,fIndex]+1)))
modFetal = model.matrix(~ pdFetal$snpPC1+
	pdFetal$snpPC2 + pdFetal$snpPC3 + pdFetal$Sex)
nsvFetal = num.sv(log2(geneRpkm[,fIndex]+1), modFetal)

snpFetal = as.matrix(snp[match(sigGene$snps,rownames(snp)),fIndex])
geneRpkmFetal = as.matrix(log2(geneRpkm[sigGene$gene,fIndex]+1))
outFetal = matrix(nrow = nrow(sigGene), nc = 3)

for(j in 1:nrow(sigGene)) {
	if(j %% 5000 == 0) cat(".")
	outFetal[j,] = summary(lm(geneRpkmFetal[j,]~snpFetal[j,] + 
		cbind(modFetal[,-1], pcaFetal$x[,1:nsvFetal])))$coef[2,c(1,3,4)]
}
colnames(outFetal) = paste0("fetal_", c("slope","tstat","pval"))
sigGene = cbind(sigGene, outFetal)
sigGene$fetal_MAF = rowSums(snpFetal, na.rm=TRUE)/(2*rowSums(!is.na(snpFetal)))

save(sigGene, file="rdas/annotated_gene_eqtl_szControl_cisOnly.rda")
