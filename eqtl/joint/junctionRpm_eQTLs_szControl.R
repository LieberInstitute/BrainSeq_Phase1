###
# qsub -t 1-23 -cwd -l mem_free=50G,h_vmem=60G,h_stack=256M -b y R CMD BATCH --no-save junctionRpm_eQTLs_adults.R
...
# covsJxn = SlicedData$new(t(cbind(mod[,-1], pcsJxn, pd2$DxNum)))
...
# save(meJxnCis, nsvJxn, pcsJxn, file="rdas/junction_eqtl_szControl_cisOnly.rda")
...
pdFetal= pd[fIndex,]
pcaFetal = prcomp(t(log2(jRpkm[expIndex,fIndex]+1)))
modFetal = model.matrix(~ pdFetal$snpPC1+
	pdFetal$snpPC2 + pdFetal$snpPC3 + pdFetal$Sex)
nsvFetal = num.sv(log2(jRpkm[expIndex,fIndex]+1), modFetal)

snpFetal = as.matrix(snp[match(sigJxn$snps,rownames(snp)),fIndex])
jRpkmFetal = as.matrix(log2(jRpkm[match(sigJxn$jxn,
	rownames(jRpkm)),fIndex]+1))
outFetal = matrix(nrow = nrow(sigJxn), nc = 3)

for(j in 1:nrow(sigJxn)) {
	if(j %% 5000 == 0) cat(".")
	outFetal[j,] = summary(lm(jRpkmFetal[j,]~snpFetal[j,] + 
		cbind(modFetal[,-1], pcaFetal$x[,1:nsvFetal])))$coef[2,c(1,3,4)]
}
colnames(outFetal) = paste0("fetal_", c("slope","tstat","pval"))
sigJxn = cbind(sigJxn, outFetal)
sigJxn$fetal_MAF = rowSums(snpFetal, na.rm=TRUE)/(2*rowSums(!is.na(snpFetal)))

save(sigJxn, file="rdas/annotated_junction_eqtl_szControl_cisOnly.rda")
