...
colnames(mcols(outEr))[7] = c("Symbol")
...
statListGR = GRangesList(c(Gene = outGene[,n], Exon = outExon[,n],
	...
	DER = outEr[,n]))
save(statListGR, file="rdas/devStats_controlSamples_RINadj.rda")

##########################
### metrics/output #######
##########################
load("rdas/devStats_controlSamples_RINadj.rda")

# number features significant
sapply(statListGR, function(x) sum(!is.na(x$p_bonf)))
sapply(statListGR, function(x) sum(x$p_bonf < 0.05, na.rm=TRUE))
sapply(statListGR, function(x) mean(x$p_bonf < 0.05, na.rm=TRUE))
sapply(statListGR, function(x) max(x$f_pval[which(x$p_bonf < 0.05)]))

# number of unique genes, by ID
sapply(statListGR, function(x) {
...
# mean expression 
t(sapply(statListGR, function(x) {
...
allGenes = unique(unlist(lapply(statListGR, function(x) 
...
geneMat = sapply(statListGR, function(x) allGenes %in% x$EnsemblGeneID)
...
## by symbol 
allGenesSym = unique(unlist(lapply(statListGR, function(x) 
...
allStatsExprs = endoapply(statListGR, function(x) 
	x[!is.na(x$p_bonf)])
